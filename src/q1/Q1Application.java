...
public class Q1Application {
    public static void main(String[] args) {
        ...
        int finalInput = input;//making input effectively final so it can be used in lambda expression.
        Thread evenInserterThread = new Thread(() -> {
            for (int i = 0; i <= finalInput; i += 2) {
                if (i != finalInput) {// this is to prevent from a deadlock to occur at i=finalInput. without this condition, the program will wait forever at i=finalinput if the input is an even number.
                    synchronized (list) {
                        list.add(i);
